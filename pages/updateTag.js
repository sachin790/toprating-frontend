...
import UpdateTagLandingPage from "../components/updateTag/landingPage";
const socket = io(API_URL);

class Ask extends React.Component {
  static async getInitialProps({ query: { id } }) {
    const res = await Axios(_GET_TOPICS);
    const specifiQuestion = await GET_SPECIFIC_QUESTION(id);
    return {
      data: res.data,
      id,
  ...
  state = {
    ...
    tag: this.props.specQuestion.tag,
    tagie: "",
    question: this.props.specQuestion.question,
    ask: false,
  ...
  handleSubmit = e => {
    ...
    let { tag, questionId, question } = this.state;

    ManageTag(questionId, tag, question)
      .then(result => {
        this.setState({
          showSnackbar: true,
          message: "Tag Added"
  ...
  render() {
    ...
    // console.log(this.props.specQuestion.question);
    ...
    return (
      <Layout
        ...
        {...this.state}
      >
        <UpdateTagLandingPage question={question} />
        <div className="cont">
