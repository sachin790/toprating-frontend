...
import { GET_SPECIFIC_TOPIC } from "../../apiRequests/apiRequests";
...
function Rec({ question, option, optionImage }) {
  const newQuestion = RemoveSpacesAddHyphen(question);
  const bgImage = optionImage ? optionImage[0] : "/static/wor.jpg";
  return (
    <>
...
function RecsView({ options, oldOption }) {
  const alternative = options.filter(item => {
...
export default class AlternativeOptions extends React.Component {
  state = {
    options: []
  };
  componentDidMount() {
